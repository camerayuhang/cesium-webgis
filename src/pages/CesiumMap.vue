...
onMounted(async () => {
  ...
  const CHN_2020_T = new Cesium.WebMapServiceImageryProvider({
    url: 'http://localhost:8080/geoserver/camerayuhang/wms',
    layers: 'camerayuhang:CHN_2020_T',
    parameters: {
      service: 'WMS',
      format: 'image/png',
      transparent: true,
  ...
  viewer.imageryLayers.addImageryProvider(CHN_2020_T);

  const Nanhui_2_mndwi_index = new Cesium.WebMapServiceImageryProvider({
    url: 'http://localhost:8080/geoserver/camerayuhang/wms',
    layers: 'camerayuhang:Nanhui_2_mndwi_index',
    parameters: {
      service: 'WMS',
      format: 'image/png',
      transparent: true,
  ...
  viewer.imageryLayers.addImageryProvider(Nanhui_2_mndwi_index);

  // set default basemap
  const imageryViewModels = viewer.baseLayerPicker.viewModel.imageryProviderViewModels;
  imageryViewModels.push(mapboxSteetViewModel);
  imageryViewModels.push(mapboxSateliteViewModel);
  viewer.baseLayerPicker.viewModel.selectedImagery = mapboxSateliteViewModel;

  // click homeButton to fly to China
  ...
  //   const url = await Cesium.IonResource.fromAssetId(963409);
  //   const newBuildingTileset = await Cesium.Cesium3DTileset.fromUrl(url, {});
